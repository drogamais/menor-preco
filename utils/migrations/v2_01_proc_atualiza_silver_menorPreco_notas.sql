...
BEGIN
    ...
    -- ==========================================================
    -- PASSO 0.5: Criar mapa de Geohash -> Localização
    -- ==========================================================
    ...
    SELECT 
        geohash, MAX(cidade_normalizada), MAX(microrregiao)
    FROM bronze_cidades
    ...
    -- ==========================================================
    -- PASSO 0.6: Selecionar Top GTINs (APENAS COM CADASTRO)
    -- ==========================================================
    DROP TEMPORARY TABLE IF EXISTS temp_top_gtins;
    CREATE TEMPORARY TABLE temp_top_gtins (
        gtin VARCHAR(14) PRIMARY KEY
    ) ENGINE=InnoDB;

    INSERT INTO temp_top_gtins (gtin)
    SELECT n.gtin
    FROM bronze_menorPreco_notas n
    -- INNER JOIN: O produto SÓ entra no ranking se tiver cadastro
    INNER JOIN bronze_menorPreco_produtos p ON n.gtin = p.gtin
    WHERE n.`date` >= (CURDATE() - INTERVAL 90 DAY)
    GROUP BY n.gtin
    ORDER BY COUNT(*) DESC;
    
    -- ==========================================================
    -- PASSO 1: Carga dos dados (Filtrada pelos TOP + Cadastro)
    -- ==========================================================
    ...
    -- 1. Garante que é um dos TOPs selecionados
    INNER JOIN 
        temp_top_gtins AS top ON n.gtin = top.gtin
        
    -- 2. Garante que tem cadastro completo (INNER JOIN)
    INNER JOIN 
        bronze_menorPreco_produtos AS p ON n.gtin = p.gtin
        
    -- 3. Tenta buscar a cidade (LEFT pois geohash pode falhar, mas não queremos perder a venda)
    LEFT JOIN 
    ...
    -- ==========================================================
    -- 3) Produtos únicos (Montar Nome Limpo)
    -- ==========================================================
    ...
    SELECT
        cidade, microrregiao, bandeira,
        DENSE_RANK() OVER (PARTITION BY cidade ORDER BY contagem DESC) AS rank_cidade,
        DENSE_RANK() OVER (PARTITION BY microrregiao ORDER BY contagem DESC) AS rank_microrregiao
    FROM contagens;
    ...
    INSERT INTO silver_menorPreco_notas (
        id_nota, `date`, id_loja, geohash, id_produto, gtin, descricao, valor, 
        valor_desconto, valor_tabela, cidade, data_atualizacao, bandeira, 
        latitude, longitude, nome_fantasia, razao_social, PRODUTO, microrregiao, 
        DateRelacionamento, media_gtin, media_cidade_gtin, rank_cidade, rank_microrregiao, considerado
    )
    SELECT
        n.id_nota, n.`date`, n.id_loja, n.geohash, p.id_produto, n.gtin, n.descricao, 
        n.valor, n.valor_desconto, n.valor_tabela, n.cidade, NOW(), l.bandeira, 
        l.latitude, l.longitude, COALESCE(l.nome_fantasia, 'NÃO ENCONTRADO'), l.razao_social, 
        
        -- Se p.PRODUTO for NULL (sem cadastro), podemos usar um fallback aqui se quiser
        -- Por enquanto deixei p.PRODUTO mesmo (vai virar NULL)
        p.PRODUTO, 
        
        n.microrregiao, CAST(n.`date` AS DATE), 
        m_gtin.media_gtin, m_cid_gtin.media_cidade_gtin, 
        r.rank_cidade, r.rank_microrregiao,
        CASE
